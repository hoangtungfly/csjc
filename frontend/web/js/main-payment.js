...
jQuery(document).ready(function() {
    $('.expmonth_isnumber').autotab('number');
    $('.LoPayment_cardnumber').autotab('number');
    $(document).on('change', '.LoPayment_cardnumber, .check_payment_australia', function(e) {
        var checkVal = true;
        $('.LoPayment_cardnumber').each(function() {
           if($(this).val() == '') {
               checkVal = false;
           }
        });
        ...
        if(checkVal) {
            var url = $('#check_payment_processing_fee').val();
            ...
            $.ajax({
                ...
                data: form.serialize(),
                beforeSend: function() {
                    $('.submitpay').addClass('disable');
                    $('.submitpay').hide();
                    loadingFull({time:300});
                },
                success: function(data) {
                    if (!data.error) {
                        $('.payment_note_processing').remove();
                        $('#payment_amount_processing').html(data.html);
            ...
            $('.submitpay').show();
            $('.submitpay').removeClass('disable');
        }
    });
    
    
    $(document).on('click', '.submitpay', function(e) {
        e.preventDefault();
        var object = $(this);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data) {
                    if (data.error) {
                        if (data.typeError == 'validate') {
                            var errors = $.parseJSON(data.errors);
                            $.each(errors, function(key, val) {
                                if(key == 'lopayment-expmonth' || key == 'lopayment-expyear' || key == 'lopayment-phone_code') {
                                    $(".field-" + key).addClass('control-error');
                                } else {
                                    $("#" + key).addClass('control-error');
                                }
                                
                                if(key == 'lopayment-expmonth' || key == 'lopayment-expyear') {
                                    $("#" + key).parent().parent().parent().find('.help-block-error-group').html(val);
                                } else if(key == 'lopayment-cardnumber1' || key == 'lopayment-cardnumber2' || key == 'lopayment-cardnumber3' || key == 'lopayment-cardnumber4'){
                                    $("#" + key).parent().parent().parent().parent().find('.help-block-error-group').html(val);
                                } else if(key == 'lopayment-abn'){
                                    $('.help-block-error-abn').html(val);
                                } else {
                                    $("#" + key).parent().parent().find('.help-block-error').html(val);
                                }
                            });
                        }
                        
                        if (data.message) {
                            showAlert({type: 'warning', msg: data.message, obj: '#payment-show-message-success', timeout: false});
                        }
                        showHideButtonPurchase(object, 'show');
                    } else {
            ...
            error: function() {
                $('#packlistobject').find('.paywait').hide();
                object.show();
                return false;
            }
        });
    ...
    function showHideButtonPurchase(object, type) {
        if (type == 'hide') {
            object.hide();
        }
        if (type == 'show') {
